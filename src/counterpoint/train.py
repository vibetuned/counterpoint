...
def train(steps=None, resume_path=None, use_bc=None, use_rnd=None, bc_coef=None, rnd_coef=None):
    ...
    if final_use_bc or final_use_rnd:
        ...
        exploration_manager = ExplorationManager(
            ...
            obs_dim=1053,  # mask(11) + grid(1040) + hand_state(1) + relative_target(1)
            device=str(device)
        )
    ...
    if exploration_manager is not None:
        ...
        # Use DiagnosticTrainer for detailed logging even without exploration
        from counterpoint.exploration import DiagnosticTrainer
        print("Using DiagnosticTrainer with detailed logging...")
        trainer = DiagnosticTrainer(cfg=cfg_agent, env=env, agents=agent)
