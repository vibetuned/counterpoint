...
class TestPhysicalDistanceMapping:
    """Test the physical distance → semitone bin → lattice span pipeline."""

    def test_lattice_step_mm(self):
        """Each lattice step should be 2 semitones worth of distance."""
        assert LATTICE_STEP_MM == pytest.approx(SEMITONE_DISTANCE_MM * 2)

    def test_lattice_span_to_mm(self):
        """1 lattice step should be ~27.5 mm."""
        mm = lattice_span_to_physical_mm(1.0)
        assert mm == pytest.approx(27.5, abs=0.1)

    def test_lattice_span_to_mm_negative(self):
        """Should use absolute value for negative spans."""
        assert lattice_span_to_physical_mm(-3.0) == lattice_span_to_physical_mm(3.0)
    ...
    def test_effective_lattice_span_passthrough(self):
        """For integer lattice spans, effective span should be close to the input."""
        # 1 lattice step = 2 semitones → bin 2 → 2/2 = 1.0
        assert physical_distance_to_effective_lattice_span(1.0) == 1.0

    def test_effective_lattice_span_larger(self):
        """Larger spans should map proportionally."""
        # 5 lattice steps = 10 semitones → bin 10 → 10/2 = 5.0
        assert physical_distance_to_effective_lattice_span(5.0) == 5.0
...
class TestJacobsLargeSpan:
    ...
    def test_no_penalty_within_maxrel(self):
        ...
        # Thumb-index MaxRel = 2.5
        assert jacobs_large_span(1, 2, 2) == 0.0
    ...
    def test_penalty_1x_for_thumb_pair(self):
        ...
        max_rel = limits[MAX_REL]  # 2.5
        # Span 1 step beyond max_rel
        cost = jacobs_large_span(1, 2, max_rel + 1)
        # Effective span should trigger 1x penalty
        assert cost > 0.0
    ...
    def test_penalty_1x_for_nonthumb_pair(self):
        ...
        max_rel = limits[MAX_REL]  # 1.0
        
        # Compare Jacobs (1x) vs Parncutt (2x) for same span
        span = max_rel + 2  # Well beyond MaxRel
...
class TestJacobsConsecutiveCost:
    ...
    def test_basic_cost(self):
        ...
        cost = calculate_jacobs_consecutive_cost(1, 10, False, 2, 11, False)
        assert cost >= 0.0
    ...
    def test_finger5_less_costly_than_parncutt(self):
        ...
        # Same transition but with finger 5
        jacobs = calculate_jacobs_consecutive_cost(1, 10, False, 5, 14, False)
        parncutt = calculate_consecutive_cost(1, 10, False, 5, 14, False)
    ...
    def test_full_cost_with_3note_context(self):
        ...
        cost = calculate_jacobs_cost(
            prev_finger=1, prev_note=10, prev_is_black=False,
            curr_finger=2, curr_note=11, curr_is_black=False,
            prev_prev_finger=3, prev_prev_note=9,
        )
        assert cost >= 0.0
